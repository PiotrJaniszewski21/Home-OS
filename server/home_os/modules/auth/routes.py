...
def fresh_session_required(f):
    ...
    def decorated(*args, **kwargs):
        if not login_fresh():
            if request.path.startswith("/api/"):
                session["next_after_reauth"] = (
                    _local_redirect_target(request.headers.get("X-Reauth-Return-To"))
                    or url_for("monitor.dashboard")
                )
                return jsonify({
                    "ok": False,
...
def edit_user(user_id):
    ...
    if request.method == "POST":
        ...
        requested_default = request.form.get("default_page", "dashboard")
        user.default_page = (
            requested_default if requested_default in valid_pages else "dashboard"
        )
        perms = [
            page for page in request.form.getlist("permissions")
