...
class User(UserMixin, db.Model):
    ...
    permissions = db.Column(db.Text, nullable=False, default="dashboard,files,storage,media,calendar,budget")
    ...
    def allowed_pages(self):
        if self.is_admin:
            return ["dashboard", "files", "storage", "media", "calendar", "budget", "network", "users", "settings"]
        return [
            page
            for page in (p.strip() for p in (self.permissions or "").split(","))
            if page and page not in {"ai", "sharing", "terminal"}
        ]
