...
final class FileProviderDomainService: ObservableObject {
    ...
    static let domainIdentifierValue = "home-os-v2"
    ...
    var domain: NSFileProviderDomain {
        NSFileProviderDomain(identifier: Self.domainIdentifier, displayName: "Home OS Drive")
    }
    ...
    func refreshRemoteChanges(using client: APIClient?, username: String) async {
        guard let manager = NSFileProviderManager(for: domain) else { return }
        ...
        do {
            let response = try await client.listDirectory(path: "/users")
            guard response.ok else {
                throw APIError.requestFailed(response.error ?? "Could not list Home OS user folders.")
            }
            ...
            await signalContainers(
                canonicalIdentifiers.filter { !fallbackValues.contains($0.rawValue) },
                using: manager
            )
        } catch {
    ...
    private func signalContainers(
        ...
        for identifier in identifiers {
            do {
                ...
                fileProviderLogger.info("Signaled File Provider container \(identifier.rawValue, privacy: .public)")
            } catch {
                fileProviderLogger.warning("Could not signal File Provider container \(identifier.rawValue, privacy: .public): \(error.localizedDescription, privacy: .public)")
            }
        }
    }
    ...
    static func remoteChangeContainerIdentifiers(
        ...
        let userDirectory = userDirectoryEntries.first(where: {
            $0.isDirectory && $0.name.compare(normalizedUsername, options: .caseInsensitive) == .orderedSame
        })
        let userPath = userDirectory.map(\.path) ?? "/users/\(normalizedUsername)"
        let normalizedPath = normalizedContainerPath(userPath)
        if normalizedPath != "/" && normalizedPath != "/users" {
            identifiers.append(NSFileProviderItemIdentifier(normalizedPath))
        }
        return identifiers
    }
    ...
    private func removeRegisteredDomainIfPresent() async throws {
        do {
            ...
            fileProviderLogger.warning("Could not list registered File Provider domains before repair; trying provider-wide removal")
        }

        do {
            try await removeAllRegisteredDomains()
            fileProviderLogger.info("All existing Home OS File Provider domains removed for repair")
            return
        } catch {
            let nsError = error as NSError
            fileProviderLogger.warning("Provider-wide Home OS domain removal failed during repair: \(nsError.domain, privacy: .public) \(nsError.code); trying direct removal")
        }
    ...
    private func removeAllRegisteredDomains() async throws {
    ...
    private func waitForCloudStorageRootURL(attempts: Int = 20) async throws -> URL {
        for attempt in 1...attempts {
            if let url = await knownCloudStorageRootURL() {
                return url
            }
    ...
    private func knownCloudStorageRootURL() async -> URL? {
