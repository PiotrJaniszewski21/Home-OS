...
final class ConnectionManager: ObservableObject {
    ...
    private var endpointResolutionTask: Task<Bool, Never>?
    ...
    init(
        ...
        self.discoverySession = discoverySession ?? URLSession(
    ...
    func refreshLiveMetrics(appState: AppState) async {
        guard let client else { return }
        do {
            let response = try await client.getMetrics()
            guard response.ok, let metrics = response.data else {
                throw APIError.requestFailed(response.error ?? "Metrics are unavailable.")
            }
            ...
            await reportConnectionFailureIfNeeded(appState: appState)
        }
    }
    ...
    private func resolveAndApplyEndpoint(appState: AppState, token: String, updateStatusMessage: Bool) async -> Bool {
        if let endpointResolutionTask {
            return await endpointResolutionTask.value
        }
        let task = Task { [weak self] in
            guard let self else { return false }
            return await performEndpointResolution(
                appState: appState,
                token: token,
                updateStatusMessage: updateStatusMessage
            )
        }
        endpointResolutionTask = task
        let result = await task.value
        endpointResolutionTask = nil
        return result
    }

    private func performEndpointResolution(appState: AppState, token: String, updateStatusMessage: Bool) async -> Bool {
    ...
    private func reportConnectionFailureIfNeeded(appState: AppState) async {
        guard hasEstablishedConnection, !didNotifyCurrentOutage, appState.notificationsEnabled else { return }
        didNotifyCurrentOutage = true
        await AppNotificationService.shared.notifyConnectionWarning(
            "The Mac app cannot reach your local server or domain. It will keep trying automatically."
        )
    }
