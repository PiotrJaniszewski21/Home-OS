...
PROVIDER_ID="uk.co.petershomenet.homeos.fileprovider"
SCHEME="HomeOS"
CONFIGURATION="Debug"

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROJECT_DIR="$ROOT_DIR/HomeOSApp"
...
text = text.replace("com.apple.ApplicationGroups.iOS", "com.apple.ApplicationGroups.Mac")
before = 'SystemCapabilities = "[\\"com.apple.ApplicationGroups.Mac\\": [\\"enabled\\": 1]]";'
after = """SystemCapabilities = {
...
if patched == text:
    print(f"warning: did not find generated App Groups capability string in {pbxproj}", file=sys.stderr)
else:
...
Apple Development team configured in HomeOSApp/project.yml and able to create
provisioning profiles for:

  - uk.co.petershomenet.homeos
  - uk.co.petershomenet.homeos.fileprovider
